...
let Interface = function (data) {
  ...
  */
   ...
   */
  this.atom = function (data, dataCallback, errorCallback) {
      ...
      if (data.validate !== false) {
        ...
        if (asset.symbol !== asset['fee-symbol']) {
          steps.unshift(
            ...
            baseBalance => { return {condition: testBaseBalance(baseBalance, data, assets[data.symbol]), message: 'Insufficient funds available for transaction: ' + balance + ' ' + asset['fee-symbol'] + '.'}; }, 'assert'
          );
        }
      }
  ...
  this.refreshAsset = function (data, dataCallback, errorCallback, progressCallback) {
    const steps = {};
    const assetList = data.hasOwnProperty('symbol') ? {[data.symbol]: 0} : assets;
    ...
    // if no balance is available set to n/a
    const handleUndefineds = data => {
      for (let symbol in assetList) {
        if (assets.hasOwnProperty(symbol) && typeof asset[symbol].balance === 'undefined') {
          asset[symbol].balance = 'n/a';
        }
      }
      dataCallback(data);
    };
    this.parallel(steps, (data) => { this.asset(undefined, handleUndefineds, errorCallback, progressCallback); }, errorCallback, progressCallback);
  };
  /**

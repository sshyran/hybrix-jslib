...
//  };
  ...
  this.save = function(data, dataCallback, errorCallback,progressCallback){
    if(typeof data !== 'object'){
      ...
      //TODO make url safe
      this.call({host:data.host,query:'/e/storage/set/'+data.key+'/'+JSON.stringify(data.value),channel:data.channel}, errorCallback,progressCallback);
    }else{
      this.sequential([
        {data:data.value}, 'encrypt',
        result => {return {host:data.host,query:'/e/storage/set/'+data.key+'/'+result,channel:data.channel,meta:data.meta};}, 'call'
      ],dataCallback, errorCallback,progressCallback);
    }
  }
  ...
  this.load = function(data, dataCallback, errorCallback,progressCallback){
    if(typeof data !== 'object'){
      ...
      //TODO unmake url safe
      this.call({host:data.host,query:'/e/storage/get/'+data.key,channel:data.channel,meta:data.meta}, errorCallback,progressCallback);
    }else{
      this.sequential([
        {host:data.host,query:'/e/storage/get/'+data.key,channel:data.channel}, 'call',
  ...
  this.meta = function(data, dataCallback, errorCallback,progressCallback){
    if(typeof data !== 'object'){
      ...
      this.call({host:data.host,query:'/e/storage/meta/'+data.key,channel:data.channel,meta:data.meta}, errorCallback,progressCallback);
    }
  }
...
* );
 */
  this.encrypt = function(data, dataCallback, errorCallbac) {
    if (!user_keys) {
      if (DEBUG) { console.error('No session available.'); }
      if (typeof errorCallback === 'function') { errorCallback('No session available.'); }
    }else{
 ...
 * @param {Object} data
 * @param {string} [data.entropy] - Not Yet working! Optional: must be a random string of at least 482 bytes, else taken from crypto.randomBytes.

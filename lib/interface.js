 ...
 */
  this.rawTransaction = function (data, dataCallback, errorCallback, progressCallback) {
    this.sequential({steps: [
      ...
      result => { return {query: '/asset/' + data.symbol + '/unspent/' + result.address + '/' + new Decimal(data.amount).plus( (data.hasOwnProperty('fee')?data.fee:assets[data.symbol].fee) ).toString() + '/' + data.target + (result.publicKey?'/'+result.publicKey:''), channel: data.channel, host: data.host}; }, 'rout',
 ...
 */
  this.transaction = function (data, dataCallback, errorCallback, progressCallback) {
    this.sequential({steps: [
      data, 'rawTransaction',
      tx => { return { query: '/asset/' + data.symbol + '/push/' + tx, channel: data.channel, host: data.host }; }, 'rout'
    ]}, dataCallback, errorCallback, progressCallback);
  };
  /**
 * TODO
 ...
 */
  ...
  this.save = function(data, dataCallback, errorCallback,progressCallback){
    if(typeof data !== 'object'){
      ...
      this.rout({host:data.host,query:'/e/storage/set/'+encodeURIComponent(data.key)+'/'+encodeURIComponent(JSON.stringify(data.value)),channel:data.channel},dataCallback , errorCallback,progressCallback);

...
let Interface = function (data) {
  ...
  */
  ...
  this.rawTransaction = function (data, dataCallback, errorCallback, progressCallback) {
    ...
    const validateConstructAndSignRawTransaction = () => {
      ...
      steps.push( // Construct and sign Transaction
        unspent => {
          return {symbol: data.symbol, target: data.target, amount: data.amount, fee: data.fee, unspent: unspent, message: data.message};
        }, 'signTransaction'
      );
   ...
   * @param {String} [data.queue=false] - whether to queue proof of work.
  ...
  this.workQueue = function (data, dataCallback, errorCallback, progressCallback) {
    const steps = {};
    for (let i = 0; i < powQueue.length; ++i) {
      steps[i] = {data: {submit: typeof data.submit === 'undefined' ? true : data.submit, ...powQueue[i]}, step: 'work'};
    }

 ...
 */
  this.rawTransaction = function (data, dataCallback, errorCallback, progressCallback) {
    ...
    this.sequential({steps: [
      ...
      result => { return {query: '/asset/' + data.symbol + '/unspent/' + result.address + '/' + calcUnspentAmount(data) + '/' + data.target + (result.publicKey?'/'+result.publicKey:''), channel: data.channel, host: data.host}; }, 'call',

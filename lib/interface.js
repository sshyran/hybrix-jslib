 ...
 */
  ...
  this.save = function(data, dataCallback, errorCallback,progressCallback){
    if(typeof data !== 'object'){
      ...
      this.rout({host:data.host,query:'/e/storage/set/'+encodeURIComponent(data.key)+'/'+encodeURIComponent(JSON.stringify(data.value).replace(/"/g, '\\\"')),channel:data.channel},dataCallback , errorCallback,progressCallback);
 ...
 * @param {Function} errorCallback - Called when an error occurs.
*/
  this.work = function(data, dataCallback, errorCallback) {
    proof.solve(data.hash,dataCallback, errorCallback);
  }

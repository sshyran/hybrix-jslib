 ...
 */
  this.signTransaction = function (data, dataCallback, errorCallback) {
    ...
    if(data.unspent && data.unspent.unspents && data.unspent.change) {
      unspentOutput = unspentAtomic(data.unspent,asset.factor);
    }
    // DEBUG: console.log(JSON.stringify(unspentOutput) + ' ' + asset.factor);
    var transactionData = {
 ...
 */
  this.rawTransaction = function (data, dataCallback, errorCallback, progressCallback) {
    this.sequential({steps: [
      ...
      result => { return {query: '/asset/' + data.symbol + '/unspent/' + result.address + '/' + new Decimal(data.amount).plus( (data.hasOwnProperty('fee')?data.fee:assets[data.symbol].fee) ).toString() + '/' + data.target + (result.publicKey?'/'+result.publicKey:''), channel: data.channel, host: data.host}; }, 'call',

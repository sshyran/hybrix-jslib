  ...
  */
  ...
  this.setPrivateKey = function (data, dataCallback, errorCallback) {
    this.addAsset(data, () => {
      ...
      if (!clientModules[baseMode].hasOwnProperty('importKeys')) {
        fail('Asset ' + data.symbol + ' does not support importing of private keys.', errorCallback);
      } else {
        let subMode = mode[1];
        const keys = clientModules[baseMode].importKeys(data);
        assets[symbol].data.keys = keys;
        assets[symbol].data.keys.mode = subMode;
        assets[symbol].data.publickey = clientModules[baseMode].publickey(assets[symbol].data.keys);
        assets[symbol].data.privatekey = clientModules[baseMode].privatekey(assets[symbol].data.keys);
        const address = clientModules[baseMode].address(assets[symbol].data.keys, dataCallback, errorCallback);
        if (typeof address !== 'undefined') {
  ...
  this.rawTransaction = function (data, dataCallback, errorCallback, progressCallback) {
    ...
    const validateConstructAndSignRawTransaction = () => {
      ...
      const steps = [
        // Retrieve unspents
        () => { return {query: '/a/' + data.symbol + '/unspent/' + asset.data.address + '/' + calcUnspentAmount(data, assets[data.symbol]) + '/' + data.target + (asset.data.publickey ? '/' + asset.data.publickey : ''), channel: data.channel, host: data.host}; }, 'rout',
  ...
  this.save = function (data, dataCallback, errorCallback, progressCallback) {
    ...
    if (data.work !== false) { // redirect to do and submit proof of work
      redirectCallback = result => this.work({hint: result.hint, difficulty: result.difficulty, host: data.host, channel: data.channel, key: data.key, submit: true}, dataCallback, errorCallback, progressCallback);
    } else {
    ...
    if (typeof data !== 'object') {
      ...
      this.rout({host: data.host, query: '/e/storage/save/' + encodeURIComponent(data.key) + '/' + encodeURIComponent(JSON.stringify(data.value).replace(/"/g, '\\"')), channel: data.channel}, redirectCallback, errorCallback, progressCallback);
    } else if (data.legacy === true) {
      this.sequential([
        {data: data.value}, 'encrypt',
        result => { return {host: data.host, query: '/e/storage/save/' + encodeURIComponent(legacy(data.key)) + '/' + result, channel: data.channel, meta: data.meta}; }, 'rout'
      ], redirectCallback, errorCallback, progressCallback);
    } else {
      this.sequential([
        {data: data.value}, 'encrypt',
        result => { return {host: data.host, query: '/e/storage/save/' + encodeURIComponent(data.key) + '/' + result, channel: data.channel, meta: data.meta}; }, 'rout'
      ], redirectCallback, errorCallback, progressCallback);
    }
  };
   ...
   * @category Storage
   * @param {Object} data
   * @param {Object} data.hint - The hint that needs to be worked.
  ...
  this.work = function (data, dataCallback, errorCallback, progressCallback) {
    ...
    proof.solve(data.hint, data.difficulty, redirectCallback, errorCallback, progressCallback);
  };

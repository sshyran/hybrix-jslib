...
if (typeof window.crypto === 'undefined') { //Needed to make waves work
 ...
 * @param {Function} errorCallback - Called when an error occurs.
 */
  this.assets = function(data, dataCallback, errorCallback){
    var result = {};
 ...
 * @param {Object} data - Contains hash to solve and difficulty level to use.
 * @param {string} data.hash - Hash to solve by proof of work.
 * @param {string} [data.difficulty] - Optional: Difficulty level to use.
 * @param {Function} dataCallback - Called when the method is succesful.
 * @param {Function} errorCallback - Called when an error occurs.
 */
 ...
 */
  this.addHost = function (data, dataCallback, errorCallback) {
    if (
      !/^(http|https):\/\/[a-z\d]([a-z\d\-]{0,61}[a-z\d])?(\.[a-z\d]([a-z\d\-]{0,61}[a-z\d])?)*(:\d*)?\/([-a-z\d%@_.~+&:]*\/)?$/i.test(data.host) && // hostnames
      !/^(http|https):\/\/((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)(:\d*)?\/([-a-z\d%@_.~+&:]*\/)?$/i.test(data.host) // ipv4 address
      // TODO ipv6
    ) { //  check if valid hostname is valid
      if (DEBUG) {
        console.error('Host not valid hostname or IPv4 : "' + data.host + '"  expectected: "protocol://hostnameOrIPv4Address:portnumber/path/"');
      }
      if (typeof errorCallback === 'function') {
        errorCallback('Host not valid hostname or IPv4 : "' + data.host + '"  expectected: "protocol://hostnameOrIPv4Address:portnumber/path/"');
      }
    } else {
 ...
 */
  ...
  this.save = function(data, dataCallback, errorCallback,progressCallback){
    if(typeof data !== 'object'){
      ...
      this.call({host:data.host,query:'/e/storage/set/'+encodeURIComponent(data.key)+'/'+encodeURIComponent(JSON.stringify(data.value)),channel:data.channel},dataCallback , errorCallback,progressCallback);
    }else if(data.legacy===true){
      this.sequential([
        {data:data.value}, 'encrypt',
        result => {return {host:data.host,query:'/e/storage/set/'+encodeURIComponent(legacy(data.key))+'/'+result,channel:data.channel,meta:data.meta};}, 'call'
      ],dataCallback, errorCallback,progressCallback);
    }else{
      this.sequential([
        {data:data.value}, 'encrypt',
        result => {return {host:data.host,query:'/e/storage/set/'+encodeURIComponent(data.key)+'/'+result,channel:data.channel,meta:data.meta};}, 'call'
      ],dataCallback, errorCallback,progressCallback);
    }
  }
  ...
  this.load = function(data, dataCallback, errorCallback,progressCallback){
    if(typeof data !== 'object'){
      ...
      this.call({host:data.host,query:'/e/storage/get/'+encodeURIComponent(data.key),channel:data.channel,meta:data.meta}, (data)=>{
        dataCallback(decodeURIComponent(data));
      },errorCallback,progressCallback);
    }else if(data.legacy===true){
      this.sequential([
        {host:data.host,query:'/e/storage/get/'+encodeURIComponent(legacy(data.key)),channel:data.channel}, 'call',
        result => {return {data: result};}, 'decrypt'
      ],dataCallback, errorCallback,progressCallback);
    }else{
      this.sequential([
        {host:data.host,query:'/e/storage/get/'+encodeURIComponent(data.key),channel:data.channel}, 'call',
        result => {return {data: result};}, 'decrypt'
      ],dataCallback, errorCallback,progressCallback);
    }
  }
  ...
  this.seek = function(data, dataCallback, errorCallback,progressCallback){
    if(typeof data !== 'object'){
      ...
      this.call({host:data.host,query:'/e/storage/seek/'+encodeURIComponent(legacy(data.key)),channel:data.channel,meta:data.meta}, (data)=>{
        dataCallback(decodeURIComponent(data));
      },errorCallback,progressCallback);
    }else{
      this.call({host:data.host,query:'/e/storage/seek/'+encodeURIComponent(data.key),channel:data.channel,meta:data.meta}, (data)=>{
        dataCallback(decodeURIComponent(data));
      },errorCallback,progressCallback);
    }
  }
  ...
  this.meta = function(data, dataCallback, errorCallback,progressCallback){
    if(typeof data !== 'object'){
      ...
      this.call({host:data.host,query:'/e/storage/meta/'+encodeURIComponent(data.key),channel:data.channel,meta:data.meta}, errorCallback,progressCallback);
    }
  }

 ...
 */
  ...
  this.import = function (data, dataCallback, errorCallback) {
    ...
    if(hybriddNodes.lenght>0 && data.check===true){
 ...
 */
  this.signTransaction = function (data, dataCallback, errorCallback) {
    ...
    var fee;
    try {
      fee = toIntLocal((data.fee || asset.fee), asset.feefactor);
  ...
  this.rawTransaction = function (data, dataCallback, errorCallback, progressCallback) {
    ...
    this.sequential({steps: [
      ...
      unspent => {
        ...
        return {symbol: data.symbol, target: data.target, amount: data.amount, fee: data.fee, unspent: unspent};
      }, 'signTransaction',
      rawtx => { return rawtx; }
 ...
 */
  this.transaction = function (data, dataCallback, errorCallback, progressCallback) {
    this.sequential({steps: [
      data, 'rawTransaction',
      rawtx => { var queryobj = { query: '/asset/' + data.symbol + '/push/' + rawtx, channel: data.channel, host: data.host }; return queryobj; }, 'rout'
    ]}, dataCallback, errorCallback, progressCallback);
  };
  /**
 * TODO
 ...
 */
  this.rout = function (data, dataCallback, errorCallback) {
    ...
    if (hybriddNodes.hasOwnProperty(host)) {
      var makeCall = () => {
        switch (data.channel) {
        case 'y' : hybriddNodes[host].yCall({query: data.query, channel: data.channel, userKeys: user_keys, connector: connector, retries:data.retries}, dataCallback, errorCallback); break;
        case 'z' : hybriddNodes[host].zCall({query: data.query, channel: data.channel, userKeys: user_keys, connector: connector, retries:data.retries}, dataCallback, errorCallback); break;
        default : hybriddNodes[host].call({query: data.query, connector: connector, retries:data.retries}, dataCallback, errorCallback); break;
        }
      };
 ...
 */
   ...
   * @param {Function} errorCallback - Ignored
   */
  this.escapeString = (data, dataCallback, errorCallback) => {
    dataCallback(escapeString(data));
  };

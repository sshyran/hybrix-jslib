var ychan = require('../common/ychan');
var zchan = require('../common/zchan');
var UrlBase64 = require('../common/crypto/urlbase64');
var CommonUtils = require('../common/index');

var hybrixdNode = function (host_) {
  ...
  */
  ...
  this.call = function (data, dataCallback, errorCallback) { // todo options: {connector,interval, timeout}
    ...
    */
    ...
    var errorCallbackWithRetries = error => {

      data.retries = (typeof data.retries === 'undefined')?3:data.retries-1;
      if(data.retries <= 0){
        if (typeof errorCallback === 'function') {
          errorCallback(error);
        }
    ...
    var httpsSocket = (host, query, dataCallback, errorCallback) => {
      ...
      if (data.hasOwnProperty('id') && data.id === 'id') {
        var followUp = () => {
          connector(host, '/proc/' + data.data, (response) => {
            ...
            //  if (typeof errorCallback === 'function') {
                errorCallbackWithRetries(parsedResponse.info);
             // }
  ...
  this.init = function (data, successCallback, errorCallback) {
    ...
    */
    ...
    console.log("NACL TRY 2");
    try{
      nonce = nacl.crypto_box_random_nonce();
    }catch(e){
      console.log("NACL CATCH 2",e);
    }
    console.log("NACL TRY 3");
    try{
      initial_session_data = CommonUtils.generateInitialSessionData(nonce);
    }catch(e){
      console.log("NACL CATCH 3",e);
    }

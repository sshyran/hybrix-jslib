let ychan = require('../common/ychan');
let zchan = require('../common/zchan');
let CommonUtils = require('../common/index');

const DEFAULT_FOLLOWUP_RETRIES = 20;
const DEFAULT_RETRIES = 3;

let hybrixdNode = function (host_) {
  ...
  */
  ...
  this.yCall = function (data, dataCallback, errorCallback) {
    ...
    */
    ...
    let generalSessionData = ychan.getGeneralSessionData({user_keys: data.userKeys, nonce: ternary_session_data.current_nonce}, step, ternary_session_data.sess_hex);
    /*
      generalSessionData = {
      sessionID,
      clientSessionSecKey,
        ...
        if (typeof result === 'object' && result.id === 'id') {
          const followUp = retries => {
            if (retries === 0) {
              ...
              this.yCall({query: '/proc/' + result.data, channel: data.channel, userKeys: data.userKeys, connector: data.connector, meta: data.meta}, (response) => {
  ...
  this.zCall = function (data, dataCallback, errorCallback) {
    ...
    */
    let encodedQuery = zchan.encode({user_keys: data.userKeys, nonce: ternary_session_data.current_nonce}, step, data.query);
    this.yCall({query: encodedQuery, channel: 'z', userKeys: data.userKeys, connector: data.connector, retries: data.retries}, encodedData => {
      let txtdata = zchan.decode_sub(encodedData);
      let result;
      ...
      if (typeof result === 'object' && result.id === 'id') {
        const followUp = retries => {
          if (retries === 0) {
            ...
            this.zCall({query: '/proc/' + result.data, channel: data.channel, userKeys: data.userKeys, connector: data.connector, meta: data.meta}, (response) => {
              if (response.hasOwnProperty('error') && response.error !== 0) {
                if (DEBUG) { console.error(response); }
                if (typeof errorCallback === 'function') {
                  errorCallback(response.info);
  ...
  this.call = function (data, dataCallback, errorCallback) { // todo options: {connector,interval, timeout}
    ...
    */
    ...
    let errorCallbackWithRetries = error => {
      ...
      if (data.retries <= 0) {
        ...
        setTimeout(() => this.call(data, dataCallback, errorCallback), 1000);
      }
    };

    let meta = !!data.meta; // meta is a boolean indicating whether to strip the meta data from a call
    let xhrSocket = (host, query, dataCallback, errorCallback) => {
      let xhr = new data.connector.XMLHttpRequest();
    ...
    let httpSocket = (host, query, dataCallback, errorCallback) => {
      data.connector.http.get(host + query, (res) => {
        // const contentType = res.headers['content-type'];

        if (res.statusCode < 200 || res.statusCode > 299) {
          let error = ('Request error: Status Code: ' + res.statusCode);
    ...
    let httpsSocket = (host, query, dataCallback, errorCallback) => {
    ...
    let localSocket = (host, query, dataCallback, errorCallback) => {
    ...
    connector(host, data.query, (response) => {
      ...
      if (resultData.hasOwnProperty('id') && resultData.id === 'id') {
        const originalRetries = data.retries || DEFAULT_FOLLOWUP_RETRIES;
        const followUp = retries => {
          if (retries === 0) {
            if (DEBUG) { console.error('Followup Retry limit (' + originalRetries + ') exceeded  for ' + host + ' ' + query); }
            errorCallback('Followup Retry limit (' + originalRetries + ') exceeded for ' + host + ' ' + query);
          } else {
            if (DEBUG) { console.log(retries + '/' + originalRetries + ' ' + host + ' ' + query); }
            connector(host, '/proc/' + resultData.data, (response) => {
              let parsedResponse;
              try {

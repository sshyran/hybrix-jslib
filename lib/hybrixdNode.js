...
const hybrixdNode = function (host_) {
  ...
  */
          ...
          const followUp = retries => {
            this.yCall({query: '/p/' + result.data, channel: data.channel, userKeys: data.userKeys, connector: data.connector, meta: data.meta}, (response) => {
              ...
              if (response.hasOwnProperty('error') && response.error !== 0) {
                ...
                if (DEBUG) { console.error('Followup timeout limit exceeded'); }
                errorCallback('Followup timeout limit exceeded');
              } else {
  ...
  this.zCall = function (data, dataCallback, errorCallback) {
    if (typeof errorCallback !== 'function') {
      if (DEBUG) { console.error('A NO ERROR FUNC!!!!'); }
    }
    ...
    */
    ...
    const errorCallbackWithRetries = error => {
      ...
      try {
        ...
        if (parsedError.hasOwnProperty('error') && parsedError.hasOwnProperty('data') && !meta) {
          errorCallback(parsedError.data);
        } else {
    ...
    const xhrSocket = (host, query, dataCallback, errorCallback) => {
      ...
      xhr.ontimeout = function (e) {
        if (DEBUG) {
          console.error('TIMEOUT!' + data.query);
        }
        errorCallback(e);
      };
    ...
    if (typeof connector === 'undefined') {
      ...
      if (DEBUG) { console.error('Error: No ' + protocol + ' request connector method available.'); }
      if (typeof errorCallback === 'function') {
        errorCallback('Error: No ' + protocol + ' request connector method available.');
      }
      return;
    }
    connector(host, data.query, (response) => {
      ...
      if (result.hasOwnProperty('id') && result.id === 'id') {
        const followUp = retries => {
          connector(host, '/p/' + result.data, (response) => {
            let parsedResponse;
            try {
            ...
            const timeout = data.timeout || parsedResponse.timeout || DEFAULT_TIMEOUT;
            if (parsedResponse.hasOwnProperty('error') && parsedResponse.error !== 0) {
              ...
              setTimeout(followUp, DEFAULT_FOLLOWUP_INTERVAL, retries + 1); // TODO parametrize, add timeout
            }
        ...
        dataCallback(meta ? result : result.data);
      }
    },
    errorCallbackWithRetries
    );

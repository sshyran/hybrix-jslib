...
exports.rawTransaction = assets => function (data, dataCallback, errorCallback, progressCallback) {
  ...
  const validateConstructAndSignRawTransaction = () => {
    ...
    if (data.hasOwnProperty('unspent')) { // Use manual unspents
      ...
      steps.push(() => { return {query: '/a/' + data.symbol + '/unspent/' + asset.data.address + '/' + calcUnspentAmount(data, assets[data.symbol]) + '/' + data.target + (asset.data.publickey ? '/' + asset.data.publickey : '') + '/' + (data.message ? data.message : ''), channel: data.channel, host: data.host}; }, 'rout');
    }

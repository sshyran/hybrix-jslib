...
function addAssetAndUpdateBalance (symbol, asset) {
  return {data: [
    ...
    balance => {
      asset.balance = balance; return balance;
      // BALANCE_FIX asset.data.balance = balance; return balance;
    }
  ],
  step: 'sequential'};
}
  ...
  const handleUndefineds = data => {
      ...
      if (assets.hasOwnProperty(symbol)) {
        ...
        if (asset.hasOwnProperty('symbols') && symbol !== 'hyd') {
          ...
          for (let subSymbol in asset.symbols) {
            ...
            // BALANCE_FIX const subBalance = assets[subSymbol].data.balance;
          ...
          // BALANCE_FIX  asset.data.balance = this.form({amount: balance.toString(), factor: asset.factor});
        }
      }
    }
    ...
    for (let symbol in assetList) {
      if (assets.hasOwnProperty(symbol)) {
        ...
        // BALANCE_FIX               if (typeof assets[symbol].data.balance === 'undefined') { // if no balance is available set to n/a

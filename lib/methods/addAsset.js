...
exports.addAsset = (assets, fail, clientModules) => function (data, dataCallback, errorCallback) {
  ...
  const handleDetails = details => {
    if (typeof details.mode !== 'string') {
      ...
      if (clientModules.hasOwnProperty(mode)) { // Client Module was already retrieved
        this.initAsset({assetDetails: details, seed: data.seed, keys: data.keys, privateKey: data.privateKey}, dataCallback, errorCallback);
      } else if (data.hasOwnProperty('clientModuleCodeBlob')) { // Use provided  clientModuleBlob
        this.initAsset({assetDetails: details, seed: data.seed, keys: data.keys, privateKey: data.privateKey, clientModuleCodeBlob: data.clientModuleCodeBlob}, dataCallback, errorCallback);
      } else { // fetch clientModule blob from host
        this.rout({host: data.host, query: '/s/deterministic/code/' + mode, channel: data.channel}, (blob) => {
          this.initAsset({assetDetails: details, seed: data.seed, keys: data.keys, privateKey: data.privateKey, clientModuleCodeBlob: blob}, dataCallback, errorCallback);
        }, errorCallback);
      }
    }
  };
  ...
  if (!assets.hasOwnProperty(data.symbol)) { // if assets has not been iniated, retrieve and initialize
    this.rout({host: data.host, query: '/a/' + data.symbol + '/details', channel: data.channel}, handleDetails, errorCallback);

...
exports.parallel = fail => function (data, dataCallback, errorCallback, progressCallback) {
  ...
  const dataSubCallback = i => result => {
    console.log('>>>DATA>>>', i, typeof result === 'string' ? JSON.stringify(result).substr(0, 50) : '');
    if (resultMarks.hasOwnProperty(i)) {
      console.log('>>>DATA DOUBLE>>>', i);
      console.log('>>>DATA PREV  >>>', i, JSON.stringify(resultData[i]));
      return;
    }
    resultProgress[i] = 1;
    resultMarks[i] = true;
  ...
  const errorSubCallback = i => e => {
    console.log('>>>ERROR >>>', i, typeof e === 'string' ? JSON.stringify(e).substr(0, 50) : '');
    if (resultMarks.hasOwnProperty(i)) {
      console.log('>>>ERROR DOUBLE>>>', i);
      console.log('>>>ERROR PREV  >>>', i, JSON.stringify(resultData[i]));
      return;
    }
    resultMarks[i] = false;
    resultProgress[i] = 1;

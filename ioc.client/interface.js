 ...
 */
  this.initAsset = function (data, dataCallback, errorCallback) {
    if (!clientModules.hasOwnProperty(data.assetDetails['keygen-base'])) { //  blob was not yet initialized
      ...
      try {
        clientModules[data.assetDetails['keygen-base']] = CommonUtils.activate(code);
        clientModuleBlobs[data.assetDetails['keygen-base']] = code;
      } catch (e) {
  ...
  this.import = function (data, dataCallback, errorCallback) {
    ...
    try {
      ...
      clientModuleBlobs[data.id] = code;
    } catch (e) {
      if (DEBUG) { console.error(e); }
      if (typeof errorCallback === 'function') {
        errorCallback(e);// TODO prepend error message

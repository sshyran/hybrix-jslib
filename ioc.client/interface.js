 ...
 */
  this.rawTransaction = function (data, dataCallback, errorCallback, progressCallback) {
    this.sequential({steps: [
      ...
      address => { return {query: '/a/' + data.symbol + '/unspent/' + address + '/' + data.amount + '/' + data.target + '/public_key', channel: data.channel, host: data.host}; }, 'call',
 ...
 */
  this.transaction = function (data, dataCallback, errorCallback, progressCallback) {
    this.sequential({steps: [
      data, 'rawTransaction',
      tx => { return { query: '/a/' + data.symbol + '/push/' + tx, channel: data.channel, host: data.host }; }, 'call'
    ]}, dataCallback, errorCallback, progressCallback);
  };
